...
passport.use(new FacebookStrategy({
  ...
  // try {
  //   if (req.user) {
  //     const userLogin = await database.User.findOne({ 'account.facebookAccessCode': profile.id });
  //     if (userLogin) {
  ...
  //       const user = await database.User.findOne({ id: req.user.id });
  //       done(null, user);
  //     }
  //   } else {
  //     const existUser = await database.User.findOne({ 'account.facebookAccessCode': profile.id });
  //     if (existUser) {
  //       done(null, existUser);
  //     } else {
